...
with third_column:
    ...
    st.sunbheader(f"{average_steps:,} / steps")
    
with fourth_column:
    st.subheader("Sleep disorders")
    st.sunbheader(f"{average_sleep_dis:,} ")
...
fig_sleep_age = px.bar(
    ...
    y="Sleep Quality",
    title="<b>Average Sleep by Age Group</b>",
...
fig_sleep_quality_dict.update_layout(
    plot_bgcolor="rgba(0,0,0,0)",
    xaxis=dict(showgrid=False)
)
...
middle_column.plotly_chart(fig_sleep_quality_dist, use_container_width=True)
